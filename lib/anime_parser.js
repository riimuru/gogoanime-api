...
export const scrapeFilter = async({ list = [], keyc, year = 2022, season = summer, page = 1 }) => {
    try {
        const filterPage = await axios.get(`
        ${BASE_URL + filter_path}?keyword%5B%5D=${keyc}&year%5B%5D=${year}&season%5B%5D=${season}&page%5B%5D=${page}
        `);
